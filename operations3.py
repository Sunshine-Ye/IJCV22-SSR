...
class non_bottleneck_1d_rn(nn.Module):
    def __init__(self, C_in, dropprob, dlt):
        super().__init__()
        chann = int(C_in/2)
        self.conv1x1_1 = nn.Conv2d(C_in, chann, 1, stride=1, padding=0, bias=True)
        self.bn1 = nn.BatchNorm2d(chann, eps=1e-03)
        self.conv3x1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(dlt, 0), bias=True, dilation=(dlt, 1))
        self.conv1x3 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, dlt), bias=True, dilation=(1, dlt))
        self.bn1x3 = nn.BatchNorm2d(chann, eps=1e-03)

        self.conv1x1_2 = nn.Conv2d(C_in, C_in-chann, 1, stride=1, padding=0, bias=True)
        self.bn2 = nn.BatchNorm2d(C_in-chann, eps=1e-03)
        self.pool = nn.MaxPool2d(3, stride=1, padding=1)

        self.conv1x1 = nn.Conv2d(C_in, C_in, 1, stride=1, padding=0, bias=True)
        self.bn = nn.BatchNorm2d(C_in, eps=1e-03)
        self.dropout = nn.Dropout2d(dropprob)

    def forward(self, input):
        output = F.relu(self.bn1(self.conv1x1_1(input)))
        output = F.relu(self.conv3x1(output))
        output = F.relu(self.bn1x3(self.conv1x3(output)))

        output1 = F.relu(self.bn2(self.conv1x1_2(input)))
        output1 = self.pool(output1)

        x = torch.cat([output, output1], dim=1)
        x = self.conv1x1(x)
        output = self.bn(x)
        if (self.dropout.p != 0):
...
class non_bottleneck_1d_r1(nn.Module):
    def __init__(self, channel, dropprob):
        super().__init__()
        chann = int(channel/rate)
        self.conv1x1_1 = nn.Conv2d(channel, chann, 1, stride=1, padding=0, bias=True)
        self.conv3x1_1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(1, 0), bias=True)
        self.conv1x3_1 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 1), bias=True)
        self.bn1 = nn.BatchNorm2d(chann, eps=1e-03)

        self.conv1x1_2 = nn.Conv2d(channel, channel-chann, 1, stride=1, padding=0, bias=True)
        self.bn2 = nn.BatchNorm2d(channel-chann, eps=1e-03)

        self.conv1x1 = nn.Conv2d(channel, channel, 1, stride=1, padding=0, bias=True)
        self.bn = nn.BatchNorm2d(channel, eps=1e-03)
        self.dropout = nn.Dropout2d(dropprob)
...
class non_bottleneck_1d_r2(nn.Module):
    def __init__(self, channel, dropprob):
        ...
        chann = int(channel/rate)
        self.conv1x1_1 = nn.Conv2d(channel, chann, 1, stride=1, padding=0, bias=True)
        self.conv3x1_1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(1 * 2, 0), bias=True, dilation=(2, 1))
        self.conv1x3_1 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 1 * 2), bias=True, dilation=(1, 2))
        self.bn1 = nn.BatchNorm2d(chann, eps=1e-03)
...
class non_bottleneck_1d_r3(nn.Module):
    def __init__(self, channel, dropprob):
        ...
        chann = int(channel/rate)
        self.conv1x1_1 = nn.Conv2d(channel, chann, 1, stride=1, padding=0, bias=True)
        self.conv3x1_1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(1 * 4, 0), bias=True, dilation=(4, 1))
        self.conv1x3_1 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 1 * 4), bias=True, dilation=(1, 4))
        self.bn1 = nn.BatchNorm2d(chann, eps=1e-03)
...
class non_bottleneck_1d_r4(nn.Module):
    def __init__(self, channel, dropprob):
        ...
        chann = int(channel/rate)
        self.conv1x1_1 = nn.Conv2d(channel, chann, 1, stride=1, padding=0, bias=True)
        self.conv3x1_1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(1 * 8, 0), bias=True, dilation=(8, 1))
        self.conv1x3_1 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 1 * 8), bias=True, dilation=(1, 8))
        self.bn1 = nn.BatchNorm2d(chann, eps=1e-03)
...
class non_bottleneck_1d_r5(nn.Module):
    def __init__(self, channel, dropprob):
        ...
        chann = int(channel/rate)
        self.conv1x1_1 = nn.Conv2d(channel, chann, 1, stride=1, padding=0, bias=True)
        self.conv3x1_1 = nn.Conv2d(chann, chann, (3, 1), stride=1, padding=(1 * 16, 0), bias=True, dilation=(16, 1))
        self.conv1x3_1 = nn.Conv2d(chann, chann, (1, 3), stride=1, padding=(0, 1 * 16), bias=True, dilation=(1, 16))
        self.bn1 = nn.BatchNorm2d(chann, eps=1e-03)
...
OPS = {
    'skip': lambda C_in, dropprob: FactorizedReduce(C_in, C_in, stride=1, slimmable=False, width_mult_list=[1.]),
    'non_bottleneck_1d_r1': lambda C_in, dropprob, dlt=1: non_bottleneck_1d_rn(C_in, dropprob, dlt),
    'non_bottleneck_1d_r2': lambda C_in, dropprob, dlt=2: non_bottleneck_1d_rn(C_in, dropprob, dlt),
    'non_bottleneck_1d_r3': lambda C_in, dropprob, dlt=4: non_bottleneck_1d_rn(C_in, dropprob, dlt),
    'non_bottleneck_1d_r4': lambda C_in, dropprob, dlt=8: non_bottleneck_1d_rn(C_in, dropprob, dlt),
    'non_bottleneck_1d_r5': lambda C_in, dropprob, dlt=16: non_bottleneck_1d_rn(C_in, dropprob, dlt),
}