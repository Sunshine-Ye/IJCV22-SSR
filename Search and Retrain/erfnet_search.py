...
class Encoder_search(nn.Module):
    def __init__(self, num_classes, switch_cell):
        ...
        self.switch_ons_cell = []
        for i in range(len(switch_cell)):
            self.switch_ons_cell.append(sum(switch_cell[i]))
        self._initialize_alphas()

    def forward(self, input, predict=True):
        weights_o = []
        for i in range(len(self.alphas)):
            tmp = torch.sigmoid(self.alphas[i])
            weights_o.append(torch.div(tmp, torch.sum(tmp)))
        ...
        for index, layer in enumerate(self.layers):
            if index == 0 or index == 6:
                ...
                output = layer(output, weights_o[cnt])
                output = F.relu(output)
                cnt += 1
    ...
    def _initialize_alphas(self):
        print(self.switch_ons_cell)
        self.alphas = []
        for num_ops in self.switch_ons_cell:
            self.alphas.append(Variable(torch.zeros(num_ops).cuda(), requires_grad=True))
