...
class MixedOp(nn.Module):
  ...
  def forward(self, x, weights_o):
    out = sum(op(x)*w_o for op, w_o in zip(self.mixed_dw_ops, weights_o))
    return out
...
class Encoder_search(nn.Module):
    ...
    def forward(self, input, predict=True):
        ...
        norm_weights = []
        for i, ons in enumerate(self.switch_ons_cell):
            norm_weights.append(torch.div(weights_o[i][:ons], torch.sum(weights_o[i][:ons])))
        ...
        for index, layer in enumerate(self.layers):
            if index == 0 or index == 6:
                ...
                output = layer(output, norm_weights[cnt])
                output = F.relu(output)
                cnt += 1
