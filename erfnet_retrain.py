...
class PSPModule(nn.Module):
    ...
    def __init__(self, features, out_features=512, sizes=(1, 2, 3, 6, 9)):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(features+len(sizes)*out_features, features, kernel_size=1, bias=False),
            nn.BatchNorm2d(features, eps=1e-03),  # default：eps=1e-05
            nn.ReLU(),
            nn.Dropout2d(0.1)
        )
...
class Encoder_retrain(nn.Module):
    def __init__(self, num_classes, switch_cell, prun_chan):
        ...
        #         nn.ReLU(),
        #     )
        # self.ppm = PSPModule(prun_chan[cnt-1][2], 32)
